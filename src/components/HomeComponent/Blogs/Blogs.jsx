          ...
          <div className="innerContainer w-[100%] px-4 md:px-[3.12rem] lg:px-[3.12rem] py-[1.5rem] bg-[#FFFFFF]/35 rounded-[.5rem] z-[10]">
            ...
            <div className="w-full  lg:hidden ">
              ...
              >
                ...
                {multipleBlogData.map((item, index) => (
                      ...
                      <div className="dottedBorder min-h-[200px] md:min-h-[280px] p-2 text-sm bg-gradient-to-r from-[rgba(94,120,143,1)] to-[rgba(240,248,255,1) justify-between gap-4 w-full h-full border-2 border-dashed border-[#D8DFE5] rounded-[.5rem] px-[1.3rem] flex flex-col ">
                        <div className="upperContainer">
                          <p className="heading text-[1rem] md:text-[1.2rem] leading-tight lg:text-[1.87rem] font-bold text-[#ffffff]">
                            {item.heading.replace(/_/g, " ").split(' ').map((word)=>word.charAt(0).toUpperCase()+word.slice(1)).join(' ')}
                          </p>
                        </div>
                        <div className="middleContainer ">
                          <p
                        dangerouslySetInnerHTML={{
                          __html: stripHtmlAndNormalize(item.description).split(" ").slice(0, 18).join(" ") + "..."
                        }}
                          className="description line-clamp-3 text-[1rem] md:text-[1rem] text-[#0E1C29] lg:text-[1rem]  leading-tight">
                  ...
                  <div className="borderContainer  w-full h-full border-[2px] border-[#D8DFE5] rounded-[.5rem] p-[0.5rem]">
                    <div className="dottedBorder justify-between w-full h-full border-2 border-dashed border-[#D8DFE5] p-2 rounded-[.5rem] px-[1.3rem] flex flex-col ">
                      ...
                      <div className="middleContainer ">
                        <p
                        dangerouslySetInnerHTML={{
                          __html: stripHtmlAndNormalize(item.description).split(" ").slice(0, 15).join(" ") + "..."
                        }}
                        className="description text-[1rem] md:text-[1rem] lg:text-[1.2rem] text-[#0E1C29]  leading-tight ">
                          {/* {item.description.split(" ").slice(0,18).join(" ")+"...."} */}
                        </p>
                      </div>
                      <div className="lowerContainer">
                      <Link key={item._id} href={`/blogs/${item.heading.toLowerCase().replace(/\s+/g, "-")}`}>
                        <button
                        
                        className="bg-[rgb(14,28,41)] text-[.5rem] md:text[1rem] lg:text-[1rem] bg-gradient-to-r from-[rgba(14,28,41,1)] to-[rgba(50,61,104,1)] text-white px-[2.37rem] py-[0.5rem] rounded-[.5rem]">
                          Read More 
                        </button>
                        </Link>
                      </div>
