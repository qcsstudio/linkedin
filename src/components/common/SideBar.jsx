...
const SideBar = () => {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      const options = sideBarData.filter((pathData) => {

        if (userData?.role !== "admin" && userData?.role !== "brand_manager" && userData?.role !== "content_creator") {

          if(userData?.role === "analyst" || userData?.role === "client_viewer"){

            if (pathData?.heading !== "Create Post" && pathData?.heading !== "Scheduled Post" && pathData?.heading !== "Accounts") {
              return pathData;
            }
  ...
  useEffect(() => {
    ...
    const options = settingsPathOption.filter((pathData) => {
      if (userData?.role !== "admin" && userData?.role !== "brand_manager") {
        // alert(userData?.role)
        if (pathData.label !== "Create User") {
          return pathData;
        }
      } else {
        return pathData;
  ...
  return (
    <div className="flex flex-col w-[100%] gap-2 py-[2rem]">
      ...
      <div className="flex  items-center justify-center rounded-2xl py-5 w-full min-w-[200px] bg-[#ffffff]/60">
        ...
        <h6 className="font-bold relative -left-2 text-[20px]"> {!sidebarVisible ? "SETTINGS" : "QUANTUM" }</h6>
      </div>
      ...
      <div className="w-[100%] min-w-[241px]  bg-white/40 rounded-xl z-[100]">
        ...
        {!sidebarVisible && (
          <div className="w-[100%] min-w-[241px] min-h-[400px]  rounded-xl p-2 z-[100]">
            ...
            {openDropdown && (
              <div className="mt-4">
                {userData && pathOption && pathOption?.map((option, index) => (
